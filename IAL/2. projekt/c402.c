...
*/
	...
	tStackP tempstck;
	SInitP(&tempstck);
	Leftmost_Preorder(RootPtr, &tempstck);
	while(SEmptyP(&tempstck) == false){
		RootPtr = STopPopP(&tempstck);
		Leftmost_Preorder(RootPtr->RPtr, &tempstck);
...
**/
	...
	tStackP tempstck;
	SInitP(&tempstck);
	Leftmost_Inorder(RootPtr, &tempstck);
	while(SEmptyP(&tempstck) == false){
		RootPtr = STopPopP(&tempstck);
		BTWorkOut(RootPtr);
			if(RootPtr->RPtr != NULL){
				Leftmost_Inorder(RootPtr->RPtr, &tempstck);
			}
	}
}
...
**/
	...
	if (*RootPtr != NULL) {
		...
		} while ((*RootPtr) != NULL || (SEmptyP(&stack) == false)); 
	}	
}
