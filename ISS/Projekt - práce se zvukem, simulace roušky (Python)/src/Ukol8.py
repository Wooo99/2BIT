...
sentencewith, fs = sf.read('../audio/Maskon_sentence.wav')
sentencewithout, fs = sf.read('../audio/Maskoff_sentence.wav')
...
sentencewithout = copy.deepcopy(sentencewithout[4500:])
...
SimulatedSentence = lfilter(Latency, [1.0], sentencewithout)
SimulatedTone = lfilter(Latency, [1.0], Maskoff_tone)


SimlatedSentence = SimulatedSentence*0.5             #Zeslabení 
out_sentence = '../audio/sin_maskon_sentence.wav'
out_tone = '../audio/sin_maskon_tone.wav'

wavio.write(out_sentence, SimulatedSentence, fs, sampwidth = 4)
wavio.write(out_tone, SimulatedTone, fs,sampwidth = 4)

plt.gca().set_title("Porovnání") 
plt.gca().set_xlabel("Délka[vzorce]")
plt.gca().set_ylabel("Frekvence")
plt.plot(SimlatedSentence, "-k",label = "Simulovaná")
plt.plot(sentencewith, "-g",label = "Nahraná")
plt.legend()
